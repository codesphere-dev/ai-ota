...
This document describes the Git workflow for contributing to the ai-ota project.
...
1. Go to https://github.com/codesphere-dev/ai-ota
...
git clone https://github.com/YOUR_USERNAME/ai-ota.git
cd ai-ota
```
...
```bash
git remote add upstream https://github.com/codesphere-dev/ai-ota.git
```
...
```bash
git remote -v
origin    https://github.com/YOUR_USERNAME/ai-ota.git (fetch)
origin    https://github.com/YOUR_USERNAME/ai-ota.git (push)
upstream  https://github.com/codesphere-dev/ai-ota.git (fetch)upstream  https://github.com/codesphere-dev/ai-ota.git (push)
```
...
3. **Important**: Ensure the base repository is `codesphere-dev/ai-ota` and the base branch is `develop`
...
```bash
git commit -m "chore(docs): add initial project documentation and git setup [WRC1]

WHY:
Provide essential baseline documentation and project configuration files to guide contributors, ensure security awareness, define code of conduct, and establish git workflow conventions.
...
```bash
git commit -m "feat(auth): implement JWT authentication system [WRC5]
...
```bash
git commit -m "fix(api): resolve CORS issue preventing client requests [#42]
...
```bash
git commit -m "test(user): add comprehensive unit tests for user service
...
3. **Set base repository to**: `codesphere-dev/ai-ota`
